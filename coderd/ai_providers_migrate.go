...
// AIProviderFromConfig is a DB-neutral, normalized description of an AI
// provider derived purely from deployment configuration (legacy env vars
// and indexed CODER_AI_GATEWAY_PROVIDER_<N>_* env vars). It carries the
// operator-controllable inputs needed to construct a runtime provider
// without consulting the database, which is what the standalone AI
// Gateway (`coder aibridge start`) relies on.
type AIProviderFromConfig struct {
	Name    string
	Type    database.AIProviderType
	BaseURL string
	// Keys holds bearer API keys for non-Bedrock providers. Bedrock
	// providers authenticate via the Bedrock settings instead.
	Keys []string
...
// ProvidersFromConfig normalizes the deployment-values AI Bridge config
// into a deduplicated, DB-neutral list of providers, reusing the exact
// same normalization logic as the database env-seed path
// (SeedAIProvidersFromEnv). It performs no database access and is safe to
// call from a standalone process that only has the deployment
// configuration available.
func ProvidersFromConfig(ctx context.Context, cfg codersdk.AIBridgeConfig, logger slog.Logger) ([]AIProviderFromConfig, error) {
	desired, err := providersFromEnv(ctx, cfg, logger)
	if err != nil {
		return nil, err
	}
	out := make([]AIProviderFromConfig, 0, len(desired))
	for _, d := range desired {
		out = append(out, AIProviderFromConfig{
			Name:    d.Name,
			Type:    d.Type,
			BaseURL: d.BaseURL,
			Keys:    d.Keys,
