		...
		eg.Go(func() error {
			err := func() error {
				...
				err := e.db.InTx(func(tx database.Store) error {
					...
					// No autostart/autostop/dormancy/delete transition is due. The
					// workspace may still be eligible for a one-time autostop reminder
					// if its running build's deadline is approaching. Reuse the
					// already-fetched build and template schedule, plus the lock and
					// transaction we already hold, so the marker stamp is protected
					// exactly like a transition. The notification is enqueued after the
					// transaction commits.
					if reason == "" {
				...
				if autostopReminderDue {
					// Enqueue after the transaction commits, matching the
					// dormancy/auto-update notification pattern. The deadline
					// label is the (stable) deadline timestamp, so the
					// notification's dedupe hash stays effective across ticks.
					// The marker was already stamped inside the transaction, so
					// this is at-most-once per deadline; on enqueue failure we
					// only log (we do not re-arm the marker).
					if _, err := e.notificationsEnqueuer.Enqueue(
						e.ctx,
						ws.OwnerID,
