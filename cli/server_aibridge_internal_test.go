...
func TestBuildProviderFromProtoSetsAPIDumpDir(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			provider: &proto.AIProvider{
				...
				Bedrock: &proto.AIProviderBedrock{
					Region:          "us-east-1",
					AccessKey:       "AKID",
					AccessKeySecret: "secret",
				},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			provider, err := buildProvider(protoToProviderSpec(tt.provider), codersdk.AIBridgeConfig{
...
func TestBuildProviderFromProtoBedrockWithoutSettings(t *testing.T) {
	...
	_, err := buildProvider(protoToProviderSpec(&proto.AIProvider{
		Enabled: true,
		Type:    string(database.AIProviderTypeBedrock),
